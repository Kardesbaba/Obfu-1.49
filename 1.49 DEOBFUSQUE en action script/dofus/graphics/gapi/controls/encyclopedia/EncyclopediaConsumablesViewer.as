class dofus.graphics.gapi.controls.encyclopedia.EncyclopediaConsumablesViewer extends dofus.graphics.gapi.core.DofusAdvancedComponent
{
   var addToQueue;
   var _eaData;
   var _lblCount;
   ...
   static var CLASS_NAME = "EncyclopediaConsumablesViewer";
   var _nMaxLevel = 1;
   function EncyclopediaConsumablesViewer()
   {
      super();
   }
   function set focusConsumable(nItemID)
   {
   ...
   function init()
   {
      super.init(false,dofus.graphics.gapi.controls.encyclopedia.EncyclopediaConsumablesViewer.CLASS_NAME);
   }
   ...
   {
      this._dgConsumables.addEventListener("itemSelected",this);
      this._dgConsumables.addEventListener("itemRollOver",this);
      this._dgConsumables.addEventListener("itemRollOut",this);
      this._filtersList.addEventListener("filterChanged",this);
      this._filtersList.addEventListener("filterClosed",this);
   }
   function initFilters()
   {
      this._filtersList.addFilter(new dofus.graphics.gapi.controls.encyclopedia.filters.FilterLabel("CONSUMABLE_TITLE"));
   ...
   function initTexts()
   {
      this._dgConsumables.columnsNames = ["",this.api.lang.getText("NAME_BIG"),this.api.lang.getText("LEVEL"),this.api.lang.getText("ITEM_TYPE")];
   ...
   function initData()
   {
      ...
      for(var k in _loc2_)
      {
         ...
         if(!(!_loc5_ && _loc5_ != undefined))
         {
            ...
            _loc8_ = _loc7_ == dofus.datacenter.Item.SUPERTYPE_CONSUMABLE && _loc6_ != dofus.datacenter.Item.TYPE_MOUNT || (dofus.datacenter.Item.isFullSoul(_loc6_) || _loc7_ == dofus.datacenter.Item.SUPERTYPE_TTG);
            if(_loc8_)
            {
               _loc9_ = Number(k);
               ...
               _loc18_ = _loc4_.ef;
               if(_loc14_ > this._nMaxLevel)
               {
                  this._nMaxLevel = _loc14_;
               }
   ...
   function updateData()
   {
      ...
      this._dgConsumables.dataProvider = _loc2_;
      this.updateCountLabel(_loc2_.length);
   }
   function getEncyclopediaConsumable(nSearchedConsumableID)
   {
      var _loc3_ = 0;
      var _loc4_;
      while(_loc3_ < this._eaData.length)
      {
         _loc4_ = this._eaData[_loc3_];
         if(nSearchedConsumableID == _loc4_.id)
         {
            return _loc4_;
         }
         _loc3_ = _loc3_ + 1;
