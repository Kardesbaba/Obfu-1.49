class dofus.graphics.gapi.controls.encyclopedia.EncyclopediaEquipmentsViewer extends dofus.graphics.gapi.core.DofusAdvancedComponent
{
   ...
   function initFilters()
   {
      ...
      this._filtersList.addFilter(new dofus.graphics.gapi.controls.encyclopedia.filters.FilterCheck("INCLUDE_ETHEREAL_WEAPON","ethereal",false,dofus.graphics.gapi.controls.encyclopedia.filters.FilterCheck.MODE_INCLUDE));
   ...
   {
      var _loc2_ = this.api.lang.getAllEffectText();
      this._eaStats = new ank.utils.ExtendedArray();
      var _loc3_;
      for(var k in _loc2_)
      {
         _loc3_ = _loc2_[k];
         if(_loc3_.f)
         {
            this._eaStats.push({label:this.api.lang.getText("CHARACTERISTIC_NAME_" + _loc3_.c),id:"c" + _loc3_.c,order:_loc3_.p,icon:dofus.datacenter.Effect.getIconNameFromID(_loc3_.c)});
         }
      }
      this._eaStats.sortOn("order",Array.NUMERIC);
      this._eaStats.reverse();
   }
   function initData()
   {
   ...
   {
      return this._eaStats;
   }
   function isEffectForFilter(nActionId)
   {
      for(var i in this._eaStats)
      {
         if(this._eaStats[i].id == nActionId)
         {
            return true;
         }
      }
