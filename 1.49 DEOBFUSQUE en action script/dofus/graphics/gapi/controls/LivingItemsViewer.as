class dofus.graphics.gapi.controls.LivingItemsViewer extends dofus.graphics.gapi.core.DofusAdvancedComponent
{
   ...
   function updateData()
   {
      ...
      this._btnDissociate.enabled = this._oItemData.isAssociateWithLivingItem;
      this._btnFeed.enabled = this._oItemData.isAssociateWithLivingItem;
      this.initTexts();
   }
   ...
   {
      this.gapi.showTooltip(new ank.utils.ExtendedString(this._pbXP.value).addMiddleChar(this.api.lang.getConfigText("THOUSAND_SEPARATOR"),3) + " / " + new ank.utils.ExtendedString(this._pbXP.maximum).addMiddleChar(this.api.lang.getConfigText("THOUSAND_SEPARATOR"),3));
   }
   function out(oEvent)
   {
