class dofus.graphics.gapi.ui.ChooseFeed extends dofus.graphics.gapi.core.DofusAdvancedComponent
{
   ...
   function updateData()
   {
      ...
      for(var k in this._eaDataProvider)
      {
         ...
         {
            if(_loc3_.type == this._aFiltersType[_loc4_] && (!_loc3_.isSpeakingItem && (!_loc3_.isCeremonial && (_loc3_.position == -1 && _loc3_.canBeExchange))))
            {
               _loc2_.push(_loc3_);
               break;
            }
   ...
   {
      this.gapi.showTooltip(oEvent.target.contentData.name,oEvent.target.contentData.style + "ToolTip");
   }
