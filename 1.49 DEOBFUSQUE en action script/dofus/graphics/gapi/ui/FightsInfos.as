class dofus.graphics.gapi.ui.FightsInfos extends dofus.graphics.gapi.core.DofusAdvancedComponent
{
   ...
   function addListeners()
   {
      ...
      this._dgFights.addEventListener("itemdblClick",this);
   ...
   function showTeamInfos(bShow,oFight)
   {
      ...
      if(bShow)
      {
         ...
         this._lstTeam2.dataProvider = oFight.team1Players;
         this._lstTeam1.dataProvider = oFight.team2Players;
      }
   }
   ...
   function itemSelected(oEvent)
   {
      ...
      var _loc9_;
      var _loc10_;
      if((_loc0_ = oEvent.target) !== this._dgFights)
      {
         if(oEvent.row.item.type == "player")
         {
            ...
            if(this.api.datacenter.Player.isAuthorized && Key.isDown(Key.SHIFT))
            {
               ...
               _loc6_ = false;
               _loc7_ = 0;
               while(_loc7_ < this._lstTeam2.dataProvider.length)
               {
                  _loc8_ = this._lstTeam2.dataProvider[_loc7_].name;
                  if(_loc8_ == _loc4_)
                  {
                     _loc6_ = true;
                  }
               ...
               if(!_loc6_)
               {
                  _loc5_ = "";
                  _loc9_ = 0;
                  while(_loc9_ < this._lstTeam1.dataProvider.length)
                  {
                     _loc10_ = this._lstTeam1.dataProvider[_loc9_].name;
                     if(_loc10_ == _loc4_)
                     {
                        _loc6_ = true;
                     }
   ...
   {
      if(this._btnJoin.enabled)
      {
         this.click({target:this._btnJoin});
      }
   }
}
