class ank.battlefield.SpriteHandler
{
   ...
   {
      var _loc4_ = this._oSprites.getItemAt(sID);
      this.hideChildrenSprites(sID,bHide);
   ...
   function hideChildrenSprites(sID,bHide)
   {
      var _loc4_ = this._oSprites.getItemAt(sID);
      if(_loc4_ == undefined)
      {
         ank.utils.Logger.err("[hideChildrenSprites] Sprite " + sID + " inexistant");
         return undefined;
      }
      if(!_loc4_.hasChilds)
      {
      ...
      var _loc6_;
      for(var sChildID in _loc5_)
      {
         _loc6_ = _loc5_[sChildID];
         _loc6_.mc.setVisible(!bHide);
      }
   }
   ...
   function setCreatureMode(bEnabled)
   {
      ...
      for(var k in _loc3_)
      {
         var oSprite = _loc3_[k];
         if(oSprite.datacenter)
         {
            if(this.api.datacenter.Game.isFight)
            {
               if(oSprite.isPendingClearing || oSprite.isClear)
               {
                  continue;
               }
               if(oSprite.isInSpellAnimation)
               {
                  if(!oSprite._pendingCreatureToggle)
                  {
                     var self = this;
                     oSprite._pendingCreatureToggle = true;
                     oSprite.sequencer.addAction(999,false,this,function(b)
                     {
                        self.setCreatureMode(b);
                        delete oSprite._pendingCreatureToggle;
                     }
                     ,[bEnabled]);
                  }
                  if(oSprite.animTimer != undefined && getTimer() - oSprite.animTimer >= oSprite.SPRITE_SEQUENCER_TIMEOUT)
                  {
                     oSprite.isInSpellAnimation = undefined;
                     delete oSprite.animTimer;
                     delete oSprite._pendingCreatureToggle;
                  }
                  continue;
               }
            ...
            if(bEnabled)
            {
               if(!oSprite.bInCreaturesMode)
               {
                  ...
                  this.api.gfx.setSpriteGfx(oSprite.id,oSprite.creatureGfxFile);
            ...
            else if(oSprite.bInCreaturesMode)
            {
               ...
               this.api.gfx.setSpriteGfx(oSprite.id,oSprite.transformGfxFile == undefined ? oSprite.baseGfxFile : oSprite.transformGfxFile);
               if(this.api.datacenter.Game.isFight && oSprite.scaleX != 100)
               {
                  _loc4_ = oSprite.scaleX;
                  this.api.gfx.setSpriteScale(oSprite.id,_loc4_,_loc4_);
               }
   ...
   function setSpriteScale(sID,nScaleX,nScaleY)
   {
      ...
      if(_loc5_ == undefined)
      {
         ank.utils.Logger.err("[setSpriteScale] Sprite " + sID + " inexistant");
         return undefined;
      }
      _loc5_.mc.setScale(nScaleX,nScaleY);
   }
   function setGlowOnPlayer(sID,nColor)
   {
      var _loc4_ = this._oSprites.getItemAt(sID);
      if(_loc4_ == undefined)
      {
         ank.utils.Logger.err("[setGlowOnPlayer] Sprite " + sID + " inexistant");
         return undefined;
      }
      _loc4_.mc.setGlowOnPlayer(nColor);
   }
   function removeGlowOnPlayer(sID)
   {
      var _loc3_ = this._oSprites.getItemAt(sID);
      if(_loc3_ == undefined)
      {
         ank.utils.Logger.err("[removeGlowOnPlayer] Sprite " + sID + " inexistant");
         return undefined;
      }
      _loc3_.mc.removeGlowOnPlayer();
   }
   function setSpriteFooterText(sID,sText)
   {
      var _loc4_ = this._oSprites.getItemAt(sID);
      if(_loc4_ == undefined)
      {
         ank.utils.Logger.err("[setSpriteFooterText] Sprite " + sID + " inexistant");
         return undefined;
      }
      _loc4_.mc.setSpriteFooterText(sText);
   }
   function refreshAllSpriteAccessories()
   {
      var _loc2_ = this._oSprites.getItems();
      var _loc3_;
      for(var sID in _loc2_)
      {
         _loc3_ = this._oSprites.getItemAt(sID);
         if(!_loc3_.isInMove)
         {
            this.api.gfx.setForcedSpriteAnim(sID,"static");
         }
      }
   }
}
