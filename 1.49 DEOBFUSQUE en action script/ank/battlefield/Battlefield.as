class ank.battlefield.Battlefield extends MovieClip
{
   ...
   function initialize(oDatacenter,sGroundFile,sObjectFile,sAccessoriesPath,api)
   {
      ...
      {
         _global.GAC = new ank.battlefield.GlobalSpriteHandler(api);
   ...
   function activateTacticMode(api,bOrig)
   {
      ...
      else if(!this.gridHandler.bGridVisible && (api.datacenter.Game.isRunning || api.kernel.OptionsManager.getOption("Grid")))
      {
         this.drawGrid();
      }
   ...
   function hideChildrenSprites(sID,bool)
   {
      if(!this.isMapBuild)
      {
         return undefined;
      }
      this.spriteHandler.hideChildrenSprites(sID,bool);
   }
   ...
   {
      this.spriteHandler.setGlowOnPlayer(sID,nColor);
   }
   function removeGlowOnPlayer(sID)
   {
      this.spriteHandler.removeGlowOnPlayer(sID);
   }
   function setSpriteFooterText(sID,sText)
   {
      this.spriteHandler.setSpriteFooterText(sID,sText);
   }
